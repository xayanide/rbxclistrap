...
const resolvePutValueName = (valueName, valueType) => {
  const isDefaultValue =
    valueName === "DEFAULT_VALUE_NAME" || valueType === "REG_DEFAULT";
  if (isDefaultValue) {
    return "";
  }
  return valueName;
...
const findChangedRegistryValues = (valuesToPut, currentRegistryItems) => {
  ...
  for (const putKeyPath in valuesToPut) {
    ...
    if (!putKeyValues) {
      logger.warn(
        "valuesToPut object is undefined. There are no values to set. Skipping...",
      );
      continue;
    }
    ...
    for (const putValueName in putKeyValues) {
      ...
      const resolvedPutValueName = resolvePutValueName(
        putValueName,
        putValueType,
      );
      const resolvedPutValueType = resolvePutValueType(
        putValueName,
        putValueType,
      );
      ...
      if (
        currentValue &&
        putValueDataLower === currentValueDataLower &&
        resolvedPutValueType === currentValueType
      ) {
        continue;
      }
      logger.warn(
        `ValueName: ${resolvedPutValueName ? resolvedPutValueName : "(Default)"}`,
      );
      logger.warn(
...
const getValuePathsWithEmptyValueNames = (registryItems) => {
  ...
  for (const keyPath in registryItems) {
    ...
    if (
      keyValues.values &&
      Object.prototype.hasOwnProperty.call(keyValues.values, "")
    ) {
...
const checkUnsetValuePaths = async (unsetValuePaths) => {
  ...
  if (defaultValuePaths.length > 0) {
    logger.warn(
      `Found paths that have default values when they should be unset!:\n${JSON.stringify(defaultValuePaths, null, 2)}`,
    );
...
const updateRegistryValues = async (
  ...
  if (
    ...
    currentRegistryItems === undefined ||
  ...
  const filteredValuesToPut = findChangedRegistryValues(
    valuesToPut,
