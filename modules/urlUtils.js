...
const testUrl = async (
  ...
  try {
    ...
    if (
      ...
      (expectedData && axiosResponse.data !== expectedData)
    ) {
      return null;
    }
    return baseUrl;
...
const findOptimalUrl = async (baseUrls, endpoint, expectedData = null) => {
  ...
  throw exceptions.length > 0
    ? exceptions[0]
    : new Error(
        "All test connection attempts failed. Check your internet connection?",
      );
};
...
async function findFastestUrl(baseUrls, endpoint, expectedData = null) {
  ...
  if (!fastestUrl) {
    throw new Error(
      "None of the base URLs have responded! Check your internet connection?",
    );
  }
