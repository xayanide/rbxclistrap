...
async function getAppSettings(
  baseUrl,
  applicationName,
  channelName = DEPLOYMENT_DEFAULT_CHANNEL,
) {
  ...
  if (
    ...
    throw new Error(
      `Failed to fetch ${applicationName} application settings from Roblox API`,
    );
  }
...
async function getBootstrapperAppSettings(baseUrl, binaryType, channelName) {
  ...
  if (!applicationName) {
    throw new Error(
      `Unable to find deployment type for binary type: ${binaryType}`,
    );
  }
