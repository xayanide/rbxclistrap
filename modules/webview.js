...
const isWebViewInstalled = async () => {
  ...
  const webViewRegistryKeys = await promisifiedRegedit.list(
...
const installEdgeWebView = async (installerPath) => {
  ...
  const webViewSetupPath = nodePath.join(
    installerPath,
  ...
  if (!isWebViewFolderExists) {
    logger.warn(
      `WebView2RuntimeInstaller folder not found in ${installerPath}`,
    );
    return;
  }
  const webviewSetupFilePath = nodePath.join(
    webViewSetupPath,
  ...
  if (!isExeAccessible) {
    logger.warn(
      `MicrosoftEdgeWebview2Setup.exe not found in ${webViewSetupPath}`,
    );
    return;
  }
  ...
  logger.info(
    `Launching with command: "${webviewSetupFilePath}" "${spawnArgs.join(" ")}"`,
  );
  try {
    await new Promise((resolve, reject) => {
      const childProcess = nodeChildProcess.spawn(
        webviewSetupFilePath,
        spawnArgs,
        {
      ...
      childProcess.on("exit", (code) => {
        if (code === 0) {
          ...
          if (code === WINGET_ERROR_CODE_ALREADY_INSTALLED) {
            logger.error(
              "Microsoft Edge WebView2 Runtime installation failed because it was already installed.",
            );
            resolve();
            return;
          }
          logger.error(
            `Microsoft Edge WebView2 Runtime installation failed with code: ${code}`,
          );
    ...
    if (!isNowInstalled) {
      logger.error(
        "Microsoft Edge WebView2 Runtime installation did not complete successfully.",
      );
      return;
    }
    ...
    logger.error(
      `Error during Microsoft Edge WebView2 installation:\n${error.message}\n${error.stack}`,
    );
  }
};
